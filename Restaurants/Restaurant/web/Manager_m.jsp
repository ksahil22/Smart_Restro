...
<%
    ...
    String waiter_name="Waiter",cashier_name="Cashier",status="UnPaid",month=new String();
    try
    {
...
<header class="main_menu home_menu">
	...
	<!-- breadcrumb start-->
      ...
      <div>
         ...
         <h2><span style="  margin-left: 200px;"> Your audit for Month of <%out.println(month+" "+date.substring(0,4));%> is following</span></h2>
          <div style="height: 25px;"></div>
          <table>
              <tr style="  background-color: darkorange; box-shadow: 2px 2px 10px blue;">
                  <th colspan="3"><%out.println(month);%></th>
                </tr>
              <tr>
                 <th >Date</th>
                <th>Start Order no</th>
...
<%
    i=1;
        while(i<32)
        {
            if(i<9)
            {
                Query="select sum(amount),order_no from bill where day(date)=0"+i+" and month(date)="+date.substring(5,7)+" and year(date)="+date.substring(0,4);
            }
            else
            {
                Query="select sum(amount),order_no from bill where day(date)="+i+" and month(date)="+date.substring(5,7)+" and year(date)="+date.substring(0,4);
            }
            rs=stmt.executeQuery(Query);
            while(rs.next())
            {
...
%>
      ...
      <div style=" height: 200px; text-align: right;font-size: 60px;  padding: 20px; padding-right: 100px;font-family: sans-serif;"> 
          Total amount of Month of <%out.println(month+" "+date.substring(0,4));%> is : <span class="bill">₹<%out.println(k);%></span></div>
      <!-- End Align Area -->
<%
        }
catch(Exception e)
